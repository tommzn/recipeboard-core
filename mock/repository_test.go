...
func (suite *RepositoryMockTestSuite) TestListRecipes() {
	...
	recipes := []core.Recipe{recipe1, recipe2}
	for _, recipe := range recipes {
		suite.Nil(repo.Set(recipe))
	}

	recipes, err := repo.List(core.BakingRecipe)
	suite.Nil(err)
	suite.Len(recipes, 2)
	suite.Equal(recipe1.Id, recipes[0].Id)
	suite.Equal(recipe2.Id, recipes[1].Id)

	recipes2, err := repo.List(core.CookingRecipe)
	suite.NotNil(err)
...
func (suite *RepositoryMockTestSuite) TestDeleteRecipe() {
	...
	recipes1, _ := repo.List(core.BakingRecipe)
	suite.Len(recipes1, 2)

	suite.Nil(repo.Delete(recipe1))
	recipes2, _ := repo.List(core.BakingRecipe)
...
func (suite *RepositoryMockTestSuite) assertRecipeIsEqual(expectedRecipe, recipe core.Recipe) {
